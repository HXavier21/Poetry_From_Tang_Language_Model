...
if __name__ == '__main__':
    mode = int(input('请选择运行模式（0：训练模式，1：计算困惑度，2：生成文本）：'))
    match mode:
        case 0:
            dataset, dataloader = prepare_data.prepare_data('poetryFromTang.txt', 100)
            model = configure_model.CharRNN(dataset.vocab_size, 128, 256, 2)
        ...
        case 1:
            ...
            model = configure_model.configure_model(dataloader.dataset.vocab_size)
        ...
        case 2:
            ...
            model = configure_model.configure_model(dataset.vocab_size)
            model.load_state_dict(checkpoint['model_state_dict'])
